    ...
    def __call__(self, 
        ...
        """
          ...
          Parameters:
           ...
           - `chat_history`: Providers may be passed a past chat context as a list of chat messages in OpenAI format (see `library.providers.ChatHistory`). 
           ...
           - `kwargs`: Any other parameters to pass the provider API call, like temperature. Parameter names are the keynames in your provider's settings_schema.
                       Only relevant if you are defining a custom settings_schema JSON to edit provider/model settings in Aggrag. 
        """
        pass
...
def provider(name: str = 'Custom Provider', 
    ...
    """
      ...
      Parameters:
     ''' 
     TODO: Aggrag settings or class boolean to be added here as one of the parameters like: 
     
     RagStoreBool(base=True, raptor=False, subqa=False) 
     or 
     AggRAG(user_id=user_id, 
                         session_id=chat_file.chat_id,
                         ragstore_bool= RagStoreBool(base=True, raptor=False, subqa=False),
                         upload_type=chat_file.upload_type.value,
                         llm_model=AzureOpenAIModelNames.gpt_35_turbo_16k.value,
                         llm_deployment= AzureOpenAIModelEngines.gpt_35_turbo_16k.value
                         )  # This aggrag object is needed for index and metadata tasks
     '''

import argparse
from library.flask_app import run_server

# Main Aggrag start
def main():
    parser = argparse.ArgumentParser(description='Aggrag command line tool')

    # Serve command
    subparsers = parser.add_subparsers(dest='serve')
    serve_parser = subparsers.add_parser('serve', help='Start Aggrag server')

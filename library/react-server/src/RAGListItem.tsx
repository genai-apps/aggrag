...
import RAGItemButtonGroup from "./RAGItemButtonGroup";
...
const percTemperature = (rag_item: LLMSpec) => {
  // Get the temp for this rag item
  const temp = rag_item.settings?.temperature as number;
  if (temp === undefined) {
    console.warn(
      `Did not find temperature setting for model ${rag_item.base_model}.`,
    );
    return 50;
  }
  // Get the range of allowed values from the spec for the underlying RAG base model
  const tempspec = getTemperatureSpecForModel(rag_item.base_model);
  if (tempspec) {
    return (temp / tempspec.maximum) * 100;
  } else {
    console.warn(
      `Did not find temperature spec for model ${rag_item.base_model}.`,
    );
    return 50;
  }
};
...
export interface RAGListItemProps {
  item: LLMSpec;
...
const RAGListItem: React.FC<RAGListItemProps> = ({
  item,
  provided,
  snapshot,
  removeCallback,
      ...
      <div>
        <CardHeader>
          {item.settings?.description ? (
            <Tooltip label={item.settings?.description} withArrow>
              <span>
...
export const RAGListItemClone: React.FC<RAGListItemProps> = ({
  item,
  provided,
  snapshot,
  hideTrashIcon,
  ...
  return (
    ...
    >
      <div>
        ...
        <RAGItemButtonGroup hideTrashIcon={hideTrashIcon} />
      </div>
    </DragItem>
  );
};

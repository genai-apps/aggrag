...
def indexRAGFiles():
    ...
    loop = get_event_loop()
    index_task = loop.run_until_complete(aggrag.create_all_index_async(documents=aggrag.documents))
...
def create_iter_folder():
    ...
    existing_iterations = [f for f in os.listdir(folder_path) if os.path.isdir(os.path.join(folder_path, f)) and f.startswith("iteration ")]
    iteration_numbers = []
    ...
    if not os.path.exists(iteration_folder_path):
        ...
        return jsonify({"message": "Folder created successfully!", "iter_folder_name": iter_folder_name}), 201
    else:
...
def copy_iteration():
    ...
    if os.path.isfile(source_file_path):
        try:
            ...
            return jsonify({"message": "Folder created and file copied successfully!", "iter_folder_name": new_iter_folder_name, "file_name": file_name}), 201
        except Exception as e:
...
def load_cforge():
    ...
    for folder in os.listdir(parent_dir):
        ...
        if os.path.isdir(folder_path) and os.path.exists(metadata_path):
            with open(metadata_path, 'r') as metadata_file:
                ...
                elif metadata.get('user_id') == aggrag_user_id or "__default"  in folder or "__Default"  in folder:
    ...
    for folder_data in folder_data_list:
        ...
        for iteration in iterations:
            ...
            folder_data_structured["subItems"].append({"label": iteration, "subItems": iteration_data})
...
def save_flow():
    ...
    try:
        ...
        return jsonify({"message": "Flow saved successfully!", "file_path": file_path, "file_name": file_name}), 200
